  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: 20, marginBottom: 20 }}>
        <div style={{ width: 80, height: 80, margin: '0 auto 12px auto', background: '#ffffff', borderRadius: 16, border: '1px solid #D8D0C5', padding: 6, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          <img src={BRAND_LOGO} alt={`${BRAND_EN} logo`} style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
        </div>
        <div style={{ fontSize: 22, fontWeight: 900, color: '#3B261B', letterSpacing: -0.5, textTransform: 'uppercase' }}>
          {BRAND_EN}
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 320 }}>
          <thead>
            <tr style={{ background: '#F5F1E9', borderRadius: 8 }}>
              ...
              <th style={{ padding: '8px 10px', textAlign: 'right', fontSize: 10, fontWeight: 800, color: '#3B261B', textTransform: 'uppercase', letterSpacing: 0.8, width: 75 }}>Rate</th>
              <th style={{ padding: '8px 10px', textAlign: 'right', fontSize: 10, fontWeight: 800, color: '#3B261B', textTransform: 'uppercase', letterSpacing: 0.8, width: 85 }}>Amount</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, idx) => {
      ...
      {(showRemarks || showPrescription) && (
        <div style={{ marginTop: 16, marginBottom: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {showRemarks && (
            <div style={{ padding: '8px 10px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 8, fontWeight: 800, color: '#888', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 3 }}>Remarks</div>
              <div style={{ fontSize: 10, color: '#374151', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{remarks?.trim()}</div>
            </div>
          )}
          {showPrescription && eyePrescription && (
            <div style={{ border: '1px solid #d8dde2', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ padding: '6px 8px', background: '#eef2f3', fontSize: 8, fontWeight: 800, color: '#3B261B', textTransform: 'uppercase', letterSpacing: 1 }}>Eye Prescription</div>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 8 }}>
                <thead><tr style={{ background: '#f8fafb' }}>{['Vision', 'OD SPH', 'OD CYL', 'OD AXIS', 'OD Vn', 'OS SPH', 'OS CYL', 'OS AXIS', 'OS Vn', 'PD', 'Lens'].map(label => <th key={label} style={{ padding: '3px 2px', border: '1px solid #d8dde2', textAlign: 'center', fontWeight: 800 }}>{label}</th>)}</tr></thead>
                <tbody>{(['distance', 'near'] as const).map(row => <tr key={row}><td style={{ padding: '4px 2px', border: '1px solid #d8dde2', fontWeight: 800, textTransform: 'uppercase' }}>{row}</td>{(['sph', 'cyl', 'axis', 'vn'] as const).map(field => <td key={`od-${field}`} style={{ padding: '4px 2px', border: '1px solid #d8dde2', textAlign: 'center' }}>{eyePrescription[row].od[field] || '-'}</td>)}{(['sph', 'cyl', 'axis', 'vn'] as const).map(field => <td key={`os-${field}`} style={{ padding: '4px 2px', border: '1px solid #d8dde2', textAlign: 'center' }}>{eyePrescription[row].os[field] || '-'}</td>)}<td style={{ padding: '4px 2px', border: '1px solid #d8dde2', textAlign: 'center' }}>{eyePrescription[row].pd || '-'}</td><td style={{ padding: '4px 2px', border: '1px solid #d8dde2', textAlign: 'center' }}>{eyePrescription[row].lensType || '-'}</td></tr>)}</tbody>
              </table>
            </div>
          )}
        </div>
      ...
      {/* ── TOTALS ───────────────────────────────────────────────── */}
      <div style={{ marginTop: 24, borderTop: '2px solid #3B261B', paddingTop: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ minWidth: 240, width: '100%', maxWidth: 300 }}>
            ...
            {discountAmount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 12, color: '#3B261B' }}>
                  Coupon{couponCode ? ` (${couponCode})` : ''}
                </span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#3B261B' }}>−{formatCurrency(discountAmount)}</span>
              </div>
            )}
            {manualDiscountAmount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 12, color: '#3B261B' }}>Manual Discount</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#3B261B' }}>−{formatCurrency(manualDiscountAmount)}</span>
              </div>
            )}
            {gstAmount > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            {effectiveDelivery === 0 && discountAmount === 0 && manualDiscountAmount === 0 && gstAmount === 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 12, color: '#666' }}>Delivery</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#3B261B' }}>FREE</span>
              </div>
            )}
            <div
              style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                borderTop: '2px solid #3B261B', paddingTop: 10, marginTop: 4,
              }}
            >
              <span style={{ fontSize: 15, fontWeight: 900, color: '#3B261B', textTransform: 'uppercase', letterSpacing: 0.5 }}>Total</span>
              <span style={{ fontSize: 20, fontWeight: 900, color: '#3B261B' }}>{formatCurrency(total)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        <div style={{ fontSize: 12, fontWeight: 800, color: '#3B261B' }}>Thank you for shopping!</div>
        {onPrintReceipt && (
          <button
            type="button"
            ...
            style={{
              ...
              background: '#3B261B', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer',
            }}
          >
            Print Receipt
          </button>
