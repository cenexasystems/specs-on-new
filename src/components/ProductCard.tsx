    ...
    >
      ...
      <motion.button
        ...
        className={`absolute right-1.5 top-1.5 z-10 hidden xl:flex h-7 w-7 items-center justify-center rounded-full border transition-colors ${
          fav ? 'border-rose-200 bg-rose-50' : 'border-[#D8D0C5] bg-white/90'
        }`}
        type="button"
      ...
      {discount > 0 && (
        <div className="absolute left-2 top-2 z-10 rounded-full bg-warm-beige\/300 px-1.5 py-0.5 text-[10px] font-black text-white">
          {discount}% OFF
        </div>
      )}
