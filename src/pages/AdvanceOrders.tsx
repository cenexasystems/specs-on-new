...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-2xl border border-[#ECE9E2] bg-white p-4 shadow-sm"><div className="flex flex-col gap-3"><label className="relative"><Search className="absolute left-3 top-3 text-[#9CA3AF]" size={17}/><input className={`${inputClass} pl-10`} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Deposit ID, customer, phone, product or status"/></label><div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide"><div className="flex gap-1.5 shrink-0">{(['all','pending','ready','completed','cancelled'] as StatusFilter[]).map(value => <button key={value} onClick={() => setStatusFilter(value)} className={`rounded-lg px-3 py-2 text-xs font-black capitalize whitespace-nowrap ${statusFilter === value ? 'bg-[#3B261B] text-white' : 'bg-[#F5F3F7] text-[#626B61]'}`}>{value}</button>)}</div><select className={`${inputClass} shrink-0 ml-auto`} value={dateFilter} onChange={e => setDateFilter(e.target.value as DateFilter)}><option value="all">All Dates</option><option value="today">Today</option><option value="week">This Week</option><option value="month">This Month</option></select></div></div></div>
