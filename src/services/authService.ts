...
const getUsers = (): LocalUser[] => {
  try {
    const raw = localStorage.getItem('specson_users') || '[]'
...
const saveUsers = (u: LocalUser[]) => localStorage.setItem('specson_users', JSON.stringify(u))
...
export const authService = {
  ...
  signUp: async (params: {
    ...
    localStorage.setItem('specson_session', newUser.id)
  ...
  signIn: async (email: string, password: string): Promise<{ user: AuthUser | null; error: string | null }> => {
    ...
    localStorage.setItem('specson_session', match.id)
  ...
  signOut: async (): Promise<void> => {
    ...
    if (USE_LOCAL_AUTH_FALLBACK) {
      localStorage.removeItem('specson_session')
    }
  },
  ...
  getCurrentUser: async (): Promise<AuthUser | null> => {
    ...
    // localStorage fallback
    const sid = localStorage.getItem('specson_session')
    if (!sid) return null
    const users = getUsers()
