      ...
      >
        ...
        <div className={`hidden lg:flex items-center relative transition-all duration-300 ${sidebarCollapsed ? 'flex-col items-center pt-5 pb-4 px-2 gap-3' : 'px-5 py-5 justify-between'}`}>
          <Link to="/pos" title="Go to Billing Panel" className={`flex items-center gap-3 min-w-0 transition-all duration-300 ${sidebarCollapsed ? 'justify-center' : 'flex-1'}`}>
            <div className="flex items-center justify-center shrink-0 w-11 h-11 rounded-xl bg-white border border-[#D8D0C5] shadow-sm overflow-hidden p-1 hover:scale-105 transition-transform">
              <img src="/specson-logo.jpg" alt="Specs On logo" className="w-full h-full object-contain" />
            </div>
            {!sidebarCollapsed && (
              <h1 className="text-[20px] font-black text-white truncate tracking-tight">Specs On</h1>
            )}
          </Link>
          <button
            type="button"
        ...
        <div className="flex lg:hidden items-center justify-between px-4 py-4 border-b border-white/10">
          <Link to="/pos" title="Go to Billing Panel" className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-[16px] font-black text-white truncate">Specs On</span>
          </Link>
        </div>
        {/* Nav */}
        <nav
              ...
              <div className="flex flex-wrap gap-2 bg-[#F3F4F6] rounded-2xl p-1.5">
                {([
                  ...
                  <button key={id} onClick={() => setPosAnalyticsTab(id as PosAnalyticsTab)}
                    className={`px-4 py-2 rounded-xl text-[12px] font-black tracking-wide transition-all ${posAnalyticsTab === id ? 'bg-white text-[#3B261B] shadow-sm' : 'text-[#6B7280] hover:text-[#111111]'}`}>
                    {label}
                  </button>
                ))}
              </div>
                    ...
                    <div key={o.id} className="rounded-2xl border border-[#FDDBB4]/60 bg-[#FBFAF6] p-3 sm:p-4 space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 pt-1">
                        ...
                        <select value={normalizeStatus(o.status)} onChange={e => void updateOrderStatus(o.id, e.target.value)}
                          className={`min-h-[44px] flex-1 cursor-pointer rounded-xl border px-3 py-2 text-[12px] font-black outline-none ${normalizeStatus(o.status) === 'completed' ? 'border-[#D8BA8A] bg-warm-beige/30 text-choc-brown' : 'border-amber-200 bg-amber-50 text-amber-700'}`}>
                          <option value="pending">{l('Pending', 'நிலுவை')}</option>
                          <option value="completed">{l('Completed', 'முடிந்தது')}</option>
                        </select>
                        {role === 'admin' && (
                          <button onClick={() => void deleteOrder(o.id, o.invoice_no)} className="h-10 w-10 sm:h-11 sm:w-11 shrink-0 rounded-xl border border-[#FDDBB4]/60 text-[#3B261B] transition-colors hover:bg-[#3B261B]/5" title="Delete Order">
